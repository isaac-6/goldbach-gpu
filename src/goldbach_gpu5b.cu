...
// v5b introduces timers for detailed breakdown of each segment's processing time, 
// including CPU sieve, GPU copy, kernel execution, and CPU fallback. 
// This allows us to identify bottlenecks and optimize further.
//
...
int main(int argc, char** argv) {
    ...
    while (seg_start <= LIMIT) {
        ...
        double ms_sieve =
            std::chrono::duration<double, std::milli>(t_sieve_end - t_sieve_start).count();

        double ms_copy_seg =
            std::chrono::duration<double, std::milli>(t_copy_seg_end - t_copy_seg_start).count();

        double ms_total =
            std::chrono::duration<double, std::milli>(t_seg_end - t_seg_start).count();

        total_ms_sieve         += ms_sieve;
        total_ms_copy_seg      += ms_copy_seg;
    ...
    // -------------------------------------------------------
    std::cout << "\n--- Timing breakdown ---\n";
    std::cout << "CPU sieve total:          " << total_ms_sieve         << " ms\n";
    std::cout << "Copy seg → GPU total:     " << total_ms_copy_seg      << " ms\n";
